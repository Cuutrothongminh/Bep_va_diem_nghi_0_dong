...
function gmapLink(lat, lng) {
  return `<a href="https://www.google.com/maps?q=${lat},${lng}" target="_blank">Xem trên Google Maps</a>`;
}
...
function initMap(allPoints) {
  ...
  allPoints.forEach(p => {
    ...
    const urlLink = gmapLink(lat, lng);
    const popupContent = `<b>${p.Ten || p.Name || ""}</b><br>${p.DiaChi || p.Address || ""}<br>${urlLink}<br>${popupPhones}`;
